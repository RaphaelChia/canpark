...
// DEBOUNCING TINGZ in case user is speed and types fast
...
/**
 * Checks if a lat lng lies within an area
 * 
 * @param {{lat,lng}} target target object containing lat and lng
 * @param {*} mapCoord directly from map.current.getBounds()
 * @returns boolean if target lies within mapcoord
  */

export const isWithinMap = (target,mapCoord) => {
  const {_ne,_sw} = mapCoord
  return target.lng < _ne.lng && target.lng > _sw.lng && target.lat >_sw.lat && target.lat <_ne.lat
}

export const SVY21 =(function(){
  ...
  /**
   * 
   * @param {decimal} Northling
   * @param {decimal} Eastling
   * @returns 
   */
